...
use MooseX::Types::Moose qw/Value Object Maybe Str Int/;
...
{ # ($foo, $bar?)
    ...
    my $expected = Tuple[Tuple[Object,Value,Maybe[Value]], Dict[]];
...

{
    ...
    my $expected = Tuple[Tuple[Object], Dict[foo => Value, bar => Optional[Value]]];
    is($meth->type_constraint->name, $expected->name);
...

{
    ...
    my $expected = Tuple[Tuple[Value, Str, subtype(Int, where { $_ % 2 == 0 })], Dict[]];
    is($meth->type_constraint->name, $expected->name);
